...
export class AppComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    getOpacity(op:number){
      ...
      return 'rgba(237, 20, 61,'+(op*2+0.3)+')'
      }
    }
        ...
        this.sentences.forEach(sentence=>{
         ...
         parsedJSON.forEach(element => {
          if(element[1]*100>this.sliderValue){
            ...
            const sentenceNEW = element[0]//.replace(/\\n/g, '<br>')
            const similarity = Math.round((element[1]*100)*100)/100
             this.sentence.unshift(new Sentence(sentences,sentenceNEW,id,similarity))
          }
         });
          })
      })
